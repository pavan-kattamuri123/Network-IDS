...
Recently, the system has been fortified with strict **Google OAuth 2.0** authentication barriers, ensuring that no unauthorized personnel can access the application’s dashboard, live tracking metrics, or phishing detector features. Furthermore, the frontend has been completely revamped with a highly premium dark-themed glassmorphism aesthetic, ensuring that the interface is as modern as the underlying machine learning models.

### Keywords
Network Intrusion Detection System (NIDS), Machine Learning, Random Forest, Flask, MongoDB Atlas, Cybersecurity, ONNX Runtime, Network Security, Google OAuth 2.0, Authentication, Glassmorphism UI.
...
- **Backend Framework:** Python Flask and Flask-SocketIO.
- **Database Architecture:** MongoDB Atlas (Cloud-based storage).
- **Communication Protocol:** RESTful API for frontend-backend interaction and bidirectional WebSockets for real-time live capture tracking.
- **Data Source Compatibility:** Support for CICIDS 2017 standard network flow formats.
- **Authentication & Security:** Google OAuth 2.0 integration (via Authlib) driving restricted `@login_required` decorators across the application.
- **User Interface Framework:** Vanilla CSS designed with extensive glassmorphism, dynamic glowing elements, and responsive charting.
